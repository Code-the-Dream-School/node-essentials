...
| 4 | Protected Routes, Validation, and Password Hashing | Students will add task create, read, update, and delete routes to the todo backend and protect them with authentication middleware that returns `401` when no user is logged in. They will enforce basic authorization so users can only access their own tasks, validate user and task request bodies with Joi using separate schemas for create and patch operations, and hash passwords with Node's built-in `crypto` module (`scrypt` with a per-user salt) instead of storing plain text. They will work with temporary in-memory storage (`global.users`, `global.tasks`, and `global.user_id`), understanding that it is a scaffold to be replaced by PostgreSQL and Prisma later. |
