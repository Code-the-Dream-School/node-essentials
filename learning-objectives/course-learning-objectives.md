...
| 3 | Express Middleware and Error Handling | Students will learn how Express processes requests through middleware and route handlers. They will write built-in, custom, and third-party middleware; organize middleware order; modify `req` and `res`; create request IDs with Node's built-in `crypto.randomUUID()`; use temporary in-memory globals as a database scaffold; add not-found and error handling middleware; create custom error classes with status codes; choose common API status codes; return consistent error responses; and use basic debugging techniques for Express applications. |
