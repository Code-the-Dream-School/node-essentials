...
If the original function returns an error in the callback, the wrapper does a reject(err) for the promise, which you can catch if you call `fnWithAsync` inside a try/catch block.
...
3. Node is not good for computationally intensive operations, like numerical calculation.  For that, you'd want C++ or Python.
